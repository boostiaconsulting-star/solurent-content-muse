...
export const debugMetaToken = createServerFn({ method: "GET" })
  .handler(async () => {
    ...
    return {
      ...
      debugToken: debug, // tipo de token, app_id, scopes, expira_en, profile_id
      me: me, // si es user token devuelve user; si es page token devuelve page
      accounts: accounts, // lista de páginas accesibles (solo funciona con user token)
    };
  });
